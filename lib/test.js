        ...
        if (step._event) {
          ...
          await new Promise((resolve, reject) => {
            const cb = (err) => {
              timer.stop();
              if (err) {
                debug(`${step.name}(error): ${err}`);
                return reject(err);
              }
              debugSilly(`${step.name}: (run ${iter}): event emitter complete`);
              return resolve();
            };
            ...
            step._event.emitter.once(step._event.eventName, function () {
              debugSilly(`${step.name}: (run ${iter}): event emitter starting`);
              const args = Object.values(arguments);
              args.push(cb);
              timer.start();
              step._event.func.apply(null, args);
            });
          });
        }
